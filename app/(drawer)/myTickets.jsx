...
export default function MyTickets() {
  ...
  const loadTickets = async () => {
    try {
      ...
      if (bookingsData.status === 'success' && bookingsData.data) {
        for (const booking of bookingsData.data) {
          if (booking.paymentStatus === 'Paid') {
            try {
              const ticketResponse = await fetch(`${API_BASE_URL}/bookings/${booking._id}/receipt`, {
                headers: { 'Authorization': `Bearer ${token}` }
              });
      ...
      if (hiringsData.status === 'success' && hiringsData.data) {
        for (const hiring of hiringsData.data) {
          if (hiring.paymentStatus === 'Paid') {
            try {
              const ticketResponse = await fetch(`${API_BASE_URL}/hiring/${hiring._id}/receipt`, {
                headers: { 'Authorization': `Bearer ${token}` }
              });
      ...
      allTickets.sort((a, b) => new Date(b.generatedAt) - new Date(a.generatedAt));
  ...
  const downloadPDF = async (ticket) => {
    try {
      ...
      const endpoint = ticket.type === 'booking' 
  ...
  const shareTicket = async (ticket) => {
    try {
      const message = `My ${ticket.type} ticket:\n\n${ticket.type === 'booking' ? 'Booking' : 'Hiring'} Number: ${ticket.bookingNumber || ticket.hiringNumber}\nDate: ${new Date(ticket.departureDate || ticket.startDate).toLocaleDateString()}\nStatus: ${ticket.status}\n\nTicket ID: ${ticket.ticketId}`;
  ...
  const renderTicketCard = (ticket) => {
    ...
    const endDate = ticket.returnDate ? new Date(ticket.returnDate) : (ticket.endDate ? new Date(ticket.endDate) : null);
        ...
        <View className="flex-row justify-between items-start mb-3">
          ...
          <View className={`px-3 py-1 rounded-full ${
            ...
            <Text className={`text-xs font-medium ${
              ...
              ticket.status === 'Completed' ? 'text-blue-600' :
              'text-gray-600'
            }`}>
              {ticket.status}
            </Text>
          ...
          <View className="flex-row items-center">
            {isBooking && ticket.passengers && (
              <>
                ...
                <Text className="ml-1 text-sm text-gray-600">
                  {ticket.passengers.length} seat{ticket.passengers.length > 1 ? 's' : ''}
                </Text>
              </>
            )}
        ...
        <View className="flex-1 bg-gray-50">
          ...
          <View className="bg-orange-500 pt-12 pb-6 px-5">
            <View className="flex-row items-center justify-between">
              ...
              <Text className="text-xl font-bold text-white">Digital Ticket</Text>
              <View className="w-8" />
            </View>
          </View>
            ...
            <View className="bg-white rounded-2xl p-6 mb-4 shadow-lg">
                ...
                <View className="space-y-3">
                  <View className="flex-row justify-between">
                    ...
                    <Text className="font-medium">
                      {isBooking ? selectedTicket.bookingNumber : selectedTicket.hiringNumber}
                    </Text>
                  </View>
                  ...
                  <View className="flex-row justify-between">
                    ...
                    <Text className="font-medium text-right flex-1 ml-4">
                      {selectedTicket.route?.source || selectedTicket.startLocation} → {selectedTicket.route?.destination || selectedTicket.endLocation}
                    </Text>
                  </View>
                  
                  <View className="flex-row justify-between">
                    <Text className="text-gray-600">{isBooking ? 'Departure:' : 'Start Date:'}</Text>
                    <Text className="font-medium">
                      {new Date(selectedTicket.departureDate || selectedTicket.startDate).toLocaleString()}
                    </Text>
                  </View>
                  
                  {(selectedTicket.returnDate || selectedTicket.endDate) && (
                    <View className="flex-row justify-between">
                      <Text className="text-gray-600">{isBooking ? 'Return:' : 'End Date:'}</Text>
                      <Text className="font-medium">
                        {new Date(selectedTicket.returnDate || selectedTicket.endDate).toLocaleString()}
                      </Text>
                    </View>
                  )}
                  ...
                  {isBooking && selectedTicket.passengers && (
                    <View className="flex-row justify-between">
                      ...
                      <Text className="font-medium">
                        {selectedTicket.selectedSeats?.outbound?.join(', ') || 'N/A'}
                      </Text>
                    </View>
                  )}
                  ...
                  <View className="flex-row justify-between">
                    ...
                    <Text className="font-bold text-orange-600">
                      ₦{(selectedTicket.totalFare || selectedTicket.totalCost)?.toLocaleString() || '0'}
                    </Text>
                  </View>
                  ...
                  <View className="flex-row justify-between">
                    ...
                    <Text className={`font-medium ${
                      ...
                      selectedTicket.status === 'Pending' ? 'text-yellow-600' :
                      selectedTicket.status === 'Completed' ? 'text-blue-600' :
                      'text-gray-600'
                    }`}>
              ...
              {isBooking && selectedTicket.passengers && (
                <View className="border-t border-gray-100 pt-4">
                  <Text className="font-semibold text-gray-800 mb-3">Passengers</Text>
            ...
            <View className="space-y-3">
              ...
              >
                ...
                <Text className="text-white font-semibold ml-2">
                  {downloadingPdf === selectedTicket.ticketId ? 'Downloading...' : 'Download PDF'}
                </Text>
              </TouchableOpacity>
              ...
              >
                ...
                <Text className="text-gray-700 font-semibold ml-2">Share Ticket</Text>
              </TouchableOpacity>
            </View>
            
            <Text className="text-xs text-gray-500 text-center mt-6 mb-4">
              Generated on {new Date(selectedTicket.generatedAt).toLocaleString()}
            </Text>
          </ScrollView>
        </View>
      </Modal>
      ...
      >
        {tickets.length === 0 ? (
          <View className="flex-1 justify-center items-center py-20">
            ...
            <Text className="text-xl font-semibold text-gray-500 mt-4">No Tickets Yet</Text>
            <Text className="text-gray-400 text-center mt-2 px-8">
              Your tickets will appear here after successful payment for bookings or hirings.
            </Text>
            <TouchableOpacity
